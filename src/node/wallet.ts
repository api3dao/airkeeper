 ...
 */
export const deriveWalletPathFromSponsorAddress = (
...
export function getMasterHDNode(config: node.Config): ethers.utils.HDNode {
  const mnemonic = node.config.getMasterKeyMnemonic(config);
  return ethers.utils.HDNode.fromMnemonic(mnemonic);
}

export function getAirnodeWallet(config: node.Config): ethers.Wallet {
  const mnemonic = node.config.getMasterKeyMnemonic(config);
